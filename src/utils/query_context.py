...
class QueryContextManager:
    ...
    def _calculate_success_score(self, 
                                ...
                                error_message: Optional[str]) -> float:
    ...
    def _normalize_question(self, question: str) -> str:
        ...
        stop_words = ['what', 'how', 'when', 'where', 'who', 'which', 'show', 'me', 'get', 'find']
        words = normalized.split()
        filtered_words = [w for w in words if w not in stop_words]
        
        return ' '.join(filtered_words[:10])  # Limit to first 10 meaningful words
    
    def _find_similar_queries(self, user_question: str, limit: int = 5) -> List[QueryPattern]:
        ...
        for question_key, patterns in self.successful_patterns.items():
            ...
            similarity = self._calculate_text_similarity(normalized_question, question_key)
    ...
    def generate_context_prompt_addition(self, context: QueryContext) -> str:
        ...
        if context.common_patterns:
            ...
            for pattern, count in list(context.common_patterns.items())[:5]:
                prompt_parts.append(f"    • {pattern} (used {count} times successfully)")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        recent_success_rate = sum(1 for q in recent_queries if q.success_score > 0.7) / len(recent_queries)
        ...
        return {
            ...
            'most_common_patterns': dict(Counter([
                pattern for patterns in self.successful_patterns.values()
                for pattern in patterns
