...
class RealEstateSchema:
    ...
    def get_table_suggestions(cls, user_query: str) -> list:
        ...
        if any(
            word in query_lower
            for word in ["city", "location", "area", "demographic", "population"]
        ):
            relevant_tables.append("locations")

        if any(
            word in query_lower for word in ["agent", "seller", "commission", "agency"]
        ):
        ...
        if any(
            word in query_lower
            for word in ["property", "house", "home", "bedrooms", "bathrooms"]
        ):
            relevant_tables.append("properties")

        if any(
            word in query_lower
            for word in ["transaction", "sale", "purchase", "sold", "bought"]
        ):
