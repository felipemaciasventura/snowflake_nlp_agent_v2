...
from src.database.schema_inspector import DatabaseContext, SchemaInspector
from src.utils.query_context import QueryContext, QueryContextManager
...
class ContextEnhancer:
    ...
    def get_enhanced_context(
        ...
        try:
            ...
            database_context = self.schema_inspector.get_comprehensive_context(
                refresh_cache=refresh_schema, include_samples=include_samples
            )
            ...
            logger.info(
                f"Enhanced context generated: quality_score={quality_score:.2f}"
            )
    ...
    def generate_enhanced_prompt(
        self, user_question: str, base_prompt: str, enhanced_context: EnhancedContext
    ) -> str:
        """
        ...
        try:
            ...
            if enhanced_context.query_context.similar_successful_queries:
                history_context = (
                    self.query_context_manager.generate_context_prompt_addition(
            ...
            domain_section = self._format_domain_insights(
                enhanced_context.domain_insights
            )
            if domain_section:
            ...
            if enhanced_context.warning_messages:
                warnings_text = "\n".join(
                    [
                        f"    ⚠️ {warning}"
                        for warning in enhanced_context.warning_messages
                    ]
                )
                enhanced_prompt_parts.append(
            ...
            quality_level = self._get_quality_level(
                enhanced_context.context_quality_score
            )
            enhanced_prompt_parts.append(
    ...
    def _analyze_question_intent(self, user_question: str) -> str:
        ...
        if any(
            word in question_lower
            for word in ["top", "bottom", "highest", "lowest", "best", "worst", "rank"]
        ):
            return "ranking"
        ...
        if any(
            word in question_lower
            for word in ["where", "with", "having", "contains", "like"]
        ):
            return "filtering"
        ...
        if any(
            word in question_lower
            for word in ["compare", "difference", "between", "vs", "versus"]
        ):
            return "comparison"
    ...
    def _suggest_relevant_tables(
        self, user_question: str, db_context: DatabaseContext
    ) -> List[str]:
    ...
    def _suggest_relevant_columns(
        ...
        for table in relevant_tables:
            for column in table.columns:
                ...
                # Data type relevance
                if (
                    any(word in question_words for word in ["count", "number"])
                    and "INT" in column.data_type
                ):
                    column_score += 1
                elif (
                    any(word in question_words for word in ["price", "cost", "amount"])
                    and "DECIMAL" in column.data_type
                ):
                    column_score += 2
                elif (
                    any(word in question_words for word in ["name", "title"])
                    and "VARCHAR" in column.data_type
                ):
                    column_score += 1
    ...
    def _estimate_query_complexity(
        self, user_question: str, suggested_tables: List[str]
    ) -> str:
        ...
        if any(
            word in question_lower
            for word in ["rank", "window", "partition", "recursive"]
        ):
            complexity_indicators["complex"] += 2
    ...
    def _calculate_context_quality(
        ...
        if db_context.tables:
            schema_score = min(
                len(db_context.tables) / 10, 1.0
            )  # Up to 10 tables = full score
            if db_context.relationships:
                schema_score += min(
                    len(db_context.relationships) / 5, 0.5
    ...
    def _generate_recommended_approach(
        ...
        if quality_level == "excellent":
            return (
                "High-confidence context available. Generate SQL using actual schema, "
                "leverage similar successful queries, and apply learned patterns."
            )

        elif quality_level == "good":
            return (
                "Good context available. Focus on verified table structures and "
                "incorporate successful query patterns where applicable."
            )

        elif quality_level == "fair":
            return (
                "Limited context available. Use available schema information but "
                "be conservative with assumptions. Recommend basic query structure."
            )

        else:
            return (
                "Minimal context available. Generate basic SQL structure and "
    ...
    def _generate_warnings(
        ...
        if not query_context.similar_successful_queries:
            warnings.append(
                "No similar successful queries found - using schema-only context"
            )
        ...
        if large_tables:
            warnings.append(
                f"Large tables detected ({', '.join(large_tables[:3])}) - consider using LIMIT"
            )

        # Missing relationships warning
        if len(db_context.tables) > 1 and not db_context.relationships:
            warnings.append(
                "No foreign key relationships detected - verify join conditions"
            )
    ...
    def _format_domain_insights(self, domain_insights: Dict[str, Any]) -> Optional[str]:
        ...
        if domain_insights.get("complexity_level"):
            parts.append(
                f"    • Estimated Complexity: {domain_insights['complexity_level']}"
            )
    ...
    def _create_fallback_context(self, user_question: str) -> EnhancedContext:
        ...
        return EnhancedContext(
            database_context=DatabaseContext(
                database_name="UNKNOWN", schema_name="UNKNOWN"
            ),
            query_context=QueryContext(),
            domain_insights={
    ...
    def get_context_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "schema_cache_status": (
                "active" if self.schema_inspector.cache_file.exists() else "empty"
            ),
