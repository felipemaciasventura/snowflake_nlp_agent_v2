...
from src.database.schema_inspector import DatabaseContext, TableInfo, ColumnInfo
...
class ContextValidator:
    ...
    def validate_enhanced_context(self, context: EnhancedContext) -> ValidationResult:
        ...
        try:
            ...
            is_valid = (
                overall_score >= self.validation_thresholds["context_quality_minimum"] and
                db_validation["has_basic_schema"] and
                not any("CRITICAL" in warning for warning in warnings)
            )
            
            logger.info(f"Context validation completed: score={overall_score:.2f}, valid={is_valid}")
    ...
    def _validate_database_context(self, db_context: DatabaseContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if not db_context.tables:
                ...
                validation["recommendations"].append("Verify database connection and schema permissions")
                return validation
            ...
            if len(db_context.tables) < self.validation_thresholds["schema_min_tables"]:
                ...
                validation["recommendations"].append("Consider expanding database schema analysis")
            ...
            for table in db_context.tables:
                # Check column count
                if len(table.columns) < self.validation_thresholds["schema_min_columns_per_table"]:
                ...
                if table.row_count and table.row_count > self.validation_thresholds["large_table_row_threshold"]:
                    large_tables.append((table.name, table.row_count))
            
            # Report table quality issues
            if tables_with_few_columns:
                validation["warnings"].append(f"Limited columns in tables: {', '.join(tables_with_few_columns[:3])}")
            
            if tables_with_no_data:
                validation["warnings"].append(f"Empty tables detected: {', '.join(tables_with_no_data[:3])}")
                validation["recommendations"].append("Consider excluding empty tables from queries")
            
            if large_tables:
                validation["recommendations"].append("Use LIMIT clauses with large tables for better performance")
            
            # Validate relationships
            validation["relationship_count"] = len(db_context.relationships)
            if (len(db_context.tables) > self.validation_thresholds["max_tables_without_relationships"] 
                and not db_context.relationships):
                validation["warnings"].append("No table relationships detected in multi-table schema")
                validation["recommendations"].append("Verify foreign key constraints or use explicit join conditions")
            
            # Check business domain detection
            if db_context.business_domain:
                validation["has_business_domain"] = True
            else:
                validation["recommendations"].append("Consider adding table/column comments to improve domain detection")
            
        except Exception as e:
            validation["warnings"].append(f"Database context validation error: {str(e)}")
    ...
    def _validate_query_context(self, query_context: QueryContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if similar_queries:
                ...
                high_quality_queries = [q for q in similar_queries if q.success_score > 0.8]
                if len(high_quality_queries) < len(similar_queries) * 0.5:
                    validation["warnings"].append("Many similar queries have low success scores")
                    validation["recommendations"].append("Review and improve query patterns")
                
                # Check for recent queries
                recent_queries = [q for q in similar_queries 
                                if (datetime.now() - q.timestamp).days < 30]
                if not recent_queries:
                    validation["warnings"].append("No recent similar queries found")
                    validation["recommendations"].append("Query patterns may be outdated")
            
            else:
                validation["recommendations"].append("No similar queries found - results will rely on schema only")
            ...
            if not query_context.common_patterns and validation["has_history"]:
                validation["warnings"].append("No common patterns extracted from query history")
    ...
    def _validate_domain_insights(self, domain_insights: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if domain_insights.get("detected_intent"):
                ...
                validation["recommendations"].append("Could not detect query intent - using generic approach")
            ...
            if suggested_tables or suggested_columns:
                ...
                validation["recommendations"].append("Review query wording to improve table/column matching")
            ...
            if complexity == "complex" and not suggested_tables:
                validation["warnings"].append("Complex query detected but no specific tables suggested")
    ...
    def _validate_context_quality(self, context: EnhancedContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if quality_score < 0.5:
                validation["warnings"].append(f"Low context quality score: {quality_score:.2f}")
                validation["recommendations"].append("Consider refreshing schema cache or improving query history")
            
            # Check for context warnings
            if context.warning_messages:
                validation["warnings"].extend([f"Context: {w}" for w in context.warning_messages])
    ...
    def generate_validation_report(self, result: ValidationResult) -> str:
        ...
        try:
            ...
            if result.validation_details:
                ...
                if db_details:
                    report_lines.append(f"  Database: {db_details.get('table_count', 0)} tables, {db_details.get('column_count', 0)} columns")
                
                query_details = result.validation_details.get("query_context", {})
                if query_details:
                    report_lines.append(f"  Query History: {query_details.get('similar_query_count', 0)} similar queries")
                
                quality_details = result.validation_details.get("quality_assessment", {})
                if quality_details:
                    report_lines.append(f"  Quality Level: {quality_details.get('quality_level', 'unknown')}")
