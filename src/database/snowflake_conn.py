...
class SnowflakeConnection:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            log_manager.add_log("⚙️ Configuración", f"Conectando a {config.SNOWFLAKE_ACCOUNT}/{config.SNOWFLAKE_DATABASE}")
            ...
            log_manager.add_log(
                "✅ Conectado", 
                f"Usuario: {result[0]}, Warehouse: {result[1]}, DB: {result[2]}, Schema: {result[3]}"
            )
    ...
    def execute_query_to_df(self, query: str):
        ...
        try:
            ...
            if not self.engine:
                log_manager.add_log("❌ Error", "Engine SQLAlchemy no disponible", "ERROR")
                return None
